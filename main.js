...
function animateProgramToProcess() {
  ...
  stages.forEach(s => {
    ...
    if (s.arrow) {
      ...
      div.innerHTML = `
        <div style="width:40px;height:2px;background:${s.color};position:relative;">
          <div style="position:absolute;right:-6px;top:-4px;color:${s.color};font-size:10px;">▶</div>
        </div>
        <div style="font-size:9px;color:${s.color};font-family:JetBrains Mono;letter-spacing:1px;">${s.label}</div>`;
    } else {
      div.style.cssText = `display:flex;flex-direction:column;align-items:center;opacity:0;`;
      div.innerHTML = `
        <div style="width:100px;height:100px;background:${s.color}12;border:2px solid ${s.border};
          border-radius:16px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;
          ${s.glow ? `box-shadow:0 0 0px ${s.border};` : ''}">
          <div style="font-size:30px;">${s.icon}</div>
          <div style="font-size:11px;font-weight:800;color:${s.color};font-family:JetBrains Mono;">${s.top}</div>
          <div style="font-size:9px;color:${s.color}80;font-family:JetBrains Mono;">${s.bot}</div>
        </div>`;
    }
    el.appendChild(div);
  });
...
function rrAuto() {
  ...
  document.getElementById('rr-log').innerHTML    =
    `Quantum = <strong>${q}</strong>: ${cs} context switches. ${
      q <= 2 ? 'Small quantum → many switches, better response time.' :
